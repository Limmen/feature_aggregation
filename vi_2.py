...
def compute_next_belief_distribution(b, u, model, B_list, b_to_index):
    """
...
def on_the_fly_value_iteration(B_list, U, model, gamma, epsilon=1e-5, verbose=False):
    """
        ...
        for i, b in enumerate(B_list):
            ...
            for a_id in U:
                ...
                successors = compute_next_belief_distribution(
                    b, a_id, model, B_list, b_to_index
                )  # => list of (j, p)
    ...
    for i, b in enumerate(B_list):
        ...
        for a_id in U:
            immediate_cost = on_the_fly_cost(b, a_id, model)
            successors = compute_next_belief_distribution(b, a_id, model, B_list, b_to_index)
            sum_future = 0.0
            for (j, p) in successors:
                sum_future += p * J[j]
            Q_values[a_id] = immediate_cost + gamma * sum_future
